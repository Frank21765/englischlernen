...
import { Calendar, Lock, Unlock, Trash2, Plus, Shield, Check, MessageSquare, Hourglass } from "lucide-react";
...
export default function Admin() {
  ...
  const load = async () => {
    ...
    const list = (profiles ?? []).map((p: any) => ({ ...p, is_admin: adminIds.has(p.user_id) }));
    setUsers(list);
    ...
    const { data: fb } = await supabase
      ...
      .order("created_at", { ascending: false });
    const nameMap = new Map(list.map((u) => [u.user_id, u.display_name]));
    setFeedback((fb ?? []).map((f: any) => ({ ...f, display_name: nameMap.get(f.user_id) ?? null })));
  };
  ...
  const activate = async (userId: string, days = 365) => {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ valid_until: until.toISOString(), access_status: "active" })
      .eq("user_id", userId);
    setBusy(null);
    if (error) toast.error(error.message);
    else { toast.success("Freigeschaltet 🎉"); load(); }
  };
  ...
  const effectiveStatus = (u: UserRow) => {
    ...
    if (u.access_status === "pending") return { label: "Wartet", variant: "outline" as const };
  ...
  const pendingUsers = users.filter((u) => !u.is_admin && u.access_status === "pending");
      ...
      <Tabs defaultValue="users">
        <TabsList>
          ...
          <TabsTrigger value="pending">
            ...
            Wartend {pendingUsers.length > 0 && <Badge variant="destructive" className="ml-1.5 h-4 px-1.5 text-[10px]">{pendingUsers.length}</Badge>}
          </TabsTrigger>
          <TabsTrigger value="feedback">
            <MessageSquare className="h-3.5 w-3.5 mr-1" />
            Feedback {feedback.length > 0 && <Badge variant="secondary" className="ml-1.5 h-4 px-1.5 text-[10px]">{feedback.length}</Badge>}
          </TabsTrigger>
        </TabsList>

        <TabsContent value="pending" className="space-y-3 mt-4">
          {pendingUsers.length === 0 ? (
            <Card className="p-8 text-center text-muted-foreground">Keine wartenden Nutzer 🎉</Card>
          ) : pendingUsers.map((u) => (
            <Card key={u.user_id} className="p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h3 className="font-semibold">{u.display_name ?? "—"}</h3>
                  <div className="text-xs text-muted-foreground">Registriert: {fmt(u.created_at)}</div>
                </div>
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" variant="hero" disabled={busy === u.user_id} onClick={() => activate(u.user_id, 365)}>
                    <Check className="h-3.5 w-3.5 mr-1" /> Freischalten (1 Jahr)
                  </Button>
                  <Button size="sm" variant="outline" disabled={busy === u.user_id} onClick={() => activate(u.user_id, 30)}>
                    1 Monat
                  </Button>
                ...
                {!u.is_admin && (
                  <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-border">
                    {u.access_status === "pending" && (
                      <Button size="sm" variant="hero" disabled={busy === u.user_id} onClick={() => activate(u.user_id, 365)}>
                        <Check className="h-3.5 w-3.5 mr-1" />Freischalten
                      </Button>
                    )}
        ...
        <TabsContent value="feedback" className="space-y-3 mt-4">
          {feedback.length === 0 ? (
            <Card className="p-8 text-center text-muted-foreground">Noch kein Feedback.</Card>
          ) : feedback.map((f) => (
            <Card key={f.id} className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
                    <strong className="text-foreground">{f.display_name ?? "—"}</strong>
                    <span>·</span>
